  ...
  return (
    <div className={'session-panel' + (focused ? ' focused' : '') + (expanded ? ' expanded' : '')}>
      <div className='session-panel-header' onDoubleClick={onDoubleClick}>
        ...
        <button className='session-panel-nav-btn' title='Back' onClick={e => { e.stopPropagation(); transportRef.current?.sendNoReply('back'); }}>
          <svg viewBox='0 0 24 24' fill='none' stroke='currentColor' strokeWidth='2' strokeLinecap='round' strokeLinejoin='round'>
            <polyline points='15 18 9 12 15 6'/>
          </svg>
        </button>
        <button className='session-panel-nav-btn' title='Reload' onClick={e => { e.stopPropagation(); transportRef.current?.sendNoReply('reload'); }}>
          <svg viewBox='0 0 24 24' fill='none' stroke='currentColor' strokeWidth='2' strokeLinecap='round' strokeLinejoin='round'>
