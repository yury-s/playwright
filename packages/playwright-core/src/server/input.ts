...
export interface RawTouchscreen {
  ...
  swipe(options: {x: number, y: number, xDistance: number, yDistance: number, speed: number, steps: number}): Promise<void>;
}
...
export class Touchscreen {
  ...
  async swipe(options: {x: number, y: number, xDistance: number, yDistance: number, speed?: number, steps?: number}, metadata?: CallMetadata) {
    if (metadata)
      metadata.point = { x: options.x, y: options.y };
    await this._raw.swipe({
      ...options,
