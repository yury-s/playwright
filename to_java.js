...
void ${name}() {`;
  ...
  content = content.replace(/\.continue\(/g, '.continue_(');
  ...
  content = content.replace(/expect\(error.message\)\.toContain\((.+)\);/g, 'assertTrue(e.getMessage().contains($1));');
  ...
  content = content.replace(/const (.+= page.waitForNavigation)/g, 'Deferred<Response> $1');
